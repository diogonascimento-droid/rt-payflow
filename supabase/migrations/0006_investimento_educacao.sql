-- Nova função: Investimento em Educação (UCB) — lançamento manual mensal de
-- investimento e leads por associação, com nota opcional por célula (usada
-- pra registrar a quebra Meta/Google, já que essa fatura não separa por
-- conta de anúncio). Mesma regra de acesso das outras telas: leitura pra
-- qualquer conta @rtpublicity.com.br, edição só pra quem está em "editores".

create table edu_unidades (
...
create table edu_meses (
  id uuid primary key default gen_random_uuid(),
  nome text not null check (nome in ('JANEIRO','FEVEREIRO','MARÇO','ABRIL','MAIO','JUNHO','JULHO','AGOSTO','SETEMBRO','OUTUBRO','NOVEMBRO','DEZEMBRO')),
  ano int not null,
  ordem int not null,
  unique (nome, ano)
);

create table edu_lancamentos (
  id uuid primary key default gen_random_uuid(),
  unidade_id uuid not null references edu_unidades(id) on delete cascade,
  mes_id uuid not null references edu_meses(id) on delete cascade,
  investimento numeric(14,2),
  leads int,
  nota_investimento text,
  nota_leads text,
  atualizado_em timestamptz not null default now(),
  unique (unidade_id, mes_id)
);

alter table edu_unidades enable row level security;
alter table edu_meses enable row level security;
alter table edu_lancamentos enable row level security;

do $$
declare
  t text;
begin
  foreach t in array array['edu_unidades', 'edu_meses', 'edu_lancamentos']
  loop
...
-- Backfill dos dados reais que já existiam na planilha "Investimento
-- Educação UCB" (jun–set/2026), extraídos das células e dos comentários
-- (quebra Meta/Google por associação/mês).
